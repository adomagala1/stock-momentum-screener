...
#         );
#
#         -- Krok 1: Upewnij się, że RLS jest włączone dla tabeli
# ALTER TABLE public.stocks_data ENABLE ROW LEVEL SECURITY;
#
# -- Krok 2: Usuń starą, problematyczną politykę, jeśli istnieje
# DROP POLICY IF EXISTS "Public access for app users" ON public.stocks_data;
#
# -- Krok 3: Stwórz nową, JAWNĄ politykę dla operacji SELECT (ODCZYT)
# -- Pozwala każdemu (kto ma anon_key lub łączy się z bazą) na odczytywanie wszystkich wierszy.
# CREATE POLICY "Allow public read access"
# ON public.stocks_data
# FOR SELECT
# USING (true);
#
# -- Krok 4: Stwórz nową, JAWNĄ politykę dla operacji INSERT (ZAPIS)
# -- Pozwala każdemu na wstawianie nowych wierszy.
# CREATE POLICY "Allow public insert access"
# ON public.stocks_data
# FOR INSERT
# WITH CHECK (true);
#
# -- Krok 5 (Opcjonalnie, dla pełnej kontroli): Polityki dla UPDATE i DELETE
...
# --- FUNKCJE POMOCNICZE (z Twoją poprawioną funkcją demo) ---
...
def render_dashboard():
    ...
    with st.expander("⚙️ Konfiguracja Połączenia z Bazami Danych",
        ...
        if st.button("🚀 Załaduj konfigurację DEMO", use_container_width=True):
            load_demo_secrets()  # Wywołanie Twojej poprawionej funkcji
...
def display_model_tab(tab_container):
    with tab_container:
        ...
        st.info(
            "Model ocenia spółki na podstawie ceny, kapitalizacji i sentymentu. Wymaga połączenia z Twoimi bazami danych.",
            icon="💡")
        ...
        if st.button("🚀 Uruchom model", type="primary", use_container_width=True):
            with st.spinner("Łączenie z bazami i uruchamianie modelu..."):
                ...
                if df_all.empty:
                    st.warning(
                        "Brak danych o spółkach w Twojej bazie. Najpierw pobierz i zapisz dane w zakładce 'Dane giełdowe'.")
                    return
                ...
                # Zakładamy, że w MongoNewsHandler masz metodę get_average_sentiment_for_tickers
                ...
                st.dataframe(df_all_sorted[['ticker', 'price', 'market_cap', 'avg_sentiment', 'potential_score']])
