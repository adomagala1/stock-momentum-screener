...
#     """Pobiera najnowsze dane giełdowe"""
...
def create_all_tables(client):
    """Tworzy wszystkie wymagane tabele w Supabase (jeśli nie istnieją)."""
    ...
    # 🔥 Wysyłamy każde zapytanie przez RPC exec_sql
    for sql in tables_sql:
        try:
            client.rpc("exec_sql", {"sql_query": sql}).execute()
            logging.info("Tabela utworzona lub już istnieje.")
        except Exception as e:
            logging.warning(f"⚠️ Błąd przy tworzeniu tabeli: {e}")
...
def update_news_and_sentiment_for_ticker(ticker: str):
    ...
    logging.info(f"🔄 Aktualizacja newsów i sentymentu dla {ticker}")
...
def sync_user_watchlist_data(user_email: str):
    """
    Synchronizuje dane użytkownika:
    - pobiera tickery z Supabase
    - ładuje dane z PostgreSQL
    - aktualizuje newsy z MongoDB
    """
    user = get_user_by_email(user_email)
    if not user:
        logging.warning(f"Użytkownik {user_email} nie istnieje w Supabase.")
        return None

    tickers = get_user_watchlist(user["id"])
    if not tickers:
        logging.info(f"Brak tickerów na watchliście użytkownika {user_email}.")
        return None
    ...
    logging.info(f"Zsynchronizowano {len(df)} tickerów dla użytkownika {user_email}")
    return df
...
def save_stock_to_pg(df: pd.DataFrame):
    """Bezpieczny zapis danych giełdowych do PostgreSQL"""
    if df.empty:
        logging.info("Brak danych do zapisania w PostgreSQL")
        return

    # Usuń kolumnę 'No', jeśli istnieje
    if 'No' in df.columns:
    ...
    # Zmień nazwy kolumn na małe litery, usuń spacje i znaki specjalne
    ...
    if not inspector.has_table(table_name):
        logging.info(f"Tabela '{table_name}' nie istnieje – tworzę nową.")
        ...
        logging.info(f"Utworzono tabelę '{table_name}' z kolumnami: {df.columns.tolist() + ['import_date']}")
    else:
        table = Table(table_name, metadata, autoload_with=pg_engine)

    # Dodaj kolumnę import_date do df
    df['import_date'] = datetime.utcnow()

    # Dopasuj df do kolumn tabeli – pomijamy brakujące w tabeli
    ...
    with pg_engine.begin() as conn:
        try:
            ...
            logging.info(f"Zapisano {len(df_to_save)} rekordów do PostgreSQL")
        except ProgrammingError as e:
            logging.error(f"Błąd przy zapisie do PostgreSQL: {e}")


def save_news_to_supabase(df: pd.DataFrame):
    """Zapisuje stocks do Supabase"""
    from app.web.supabase_client import supabase
    if df.empty:
        st.error("Brak danych do zapisania w Supabase")
        return
...
def save_news_to_mongo(df: pd.DataFrame, ticker: str):
    ...
    logging.info(f"Zapisano/aktualizowano {len(df)} newsów w MongoDB")


def save_user_model_results(user_id: str, df: pd.DataFrame):
    """Zapisuje wyniki modelu użytkownika do Supabase"""
    from app.web.supabase_client import supabase
    if df.empty:
        return
    df['user_id'] = user_id
    ...
    logging.info(f"Zapisano wyniki modelu dla {len(df)} tickerów w Supabase")
